...
#define N 12
u16 Distance_Station_One,Distance_Station_Two,Distance_Station_Three,Distance_Station_Four,Distance_Station_Five,Distance_Station_Six,d_7,d_8,d_9,d_10,d_11,d_12,d_13,d_14;//距离
...
float jd[2];
float jd_v[2];
int flag_cs1=0,flag_cs2=0,flag_cs=0;
u8 jd_R[2];
u8 rx[4];
u8 rx_num1=0;
u8 NUM=0;
...
int main(void)
{
  ...
  while(1)
	{		
		...
		Dwm1000_Start(&Distance_Station_Two,1);//基站二		
		Dwm1000_Start(&Distance_Station_One,2);//与基站一通信
		Dwm1000_Start(&Distance_Station_Five,3);//与基站一通信
	  Dwm1000_Start(&Distance_Station_Six,4);//与基站一通信
